...
# Result folders to run tests for
RESULT_FOLDERS=(result1 result2 result3 result4)
...
# Loop through each result folder
for result_folder in "${RESULT_FOLDERS[@]}"; do
    echo ""
    echo "=============================================="
    echo "Running tests for: $result_folder"
    echo "=============================================="
    
    # Create output directories if they don't exist
    mkdir -p results/${result_folder}/output
    mkdir -p results/${result_folder}/csv

    # Create CSV header if file doesn't exist
    CSV_FILE="results/${result_folder}/csv/results.csv"
    if [ ! -f "$CSV_FILE" ]; then
        ...
        for size in "${SIZES[@]}"; do
            for zeros in "${ZEROS[@]}"; do
                for mult in "${MULTIPLICATIONS[@]}"; do
                    OUTPUT_FILE="results/${result_folder}/output/n${np}_s${size}_z${zeros}_m${mult}.txt"
    ...
    echo ""
    echo "Completed tests for: $result_folder"
    echo "Results saved to: $CSV_FILE"
done

echo ""
echo "=============================================="
echo "All tests completed for all result folders!"
echo "=============================================="
echo "Result folders: ${RESULT_FOLDERS[*]}"
echo "Output saved to: results/result{1,2,3,4}/csv/results.csv"
